...
import ai.jaebong.minecast.client.audio.AudioBuffer;
import ai.jaebong.minecast.client.audio.AudioPlayer;
...
public class AudioPacketHandler {
    ...
    private void handlePacket(byte[] data) throws IOException {
        ...
        switch (type) {
            case TYPE_START -> {
                in.readInt(); // totalBytes (프로토콜 필드, 미사용)
                int chunkCount = in.readInt();
                if (buffer == null) {
                    buffer = new AudioBuffer(chunkCount);
                } else {
                    buffer.reset(chunkCount);
                }
                player.stopAndCleanup();
                LOGGER.debug("[MineCast] START: {}chunks", chunkCount);
            }
            case TYPE_CHUNK -> {
